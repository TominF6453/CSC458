...
#define DEFAULT_TCP_ESTB_TO 7440
#define DEFAULT_TCP_TRNS_TO 300
...
int main(int argc, char **argv)
{
	...
	/* NAT additions */
	struct sr_nat nat;
	int nat_active = 0; /* Off by default */
	unsigned int icmp_to = DEFAULT_ICMP_TIMEOUT;
	unsigned int tcp_estb_to = DEFAULT_TCP_ESTB_TO;
	unsigned int tcp_trns_to = DEFAULT_TCP_TRNS_TO;
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:n:I:E:R")) != EOF)
    {
        switch (c)
        {
			...
			case 'n': /*Activate*/
				nat_active = 1;
				break;
			case 'I': /*Set ICMP query timeout*/
				icmp_to = atoi((char *) optarg);
				break;
			case 'E': /*Set ESTB timeout*/
				tcp_estb_to = atoi((char *) optarg);
				break;
			case 'R': /*Set TRNS timeout*/
				tcp_trns_to = atoi((char *) optarg);
				break;
        } /* switch */
    } /* -- while -- */
	...
	if (nat_active) {
		...
		sr.nat = nat;
		nat.icmp_to = icmp_to;
		nat.tcp_estb_to = tcp_estb_to;
		nat.tcp_trns_to = tcp_trns_to;		
		/*nat.ext_ip = sr_get_interface(&(sr), NAT_EXTERNAL_IF)->ip;*/
	}
